...
- 生产 chapter map 可直接定位 1,133/1,155 个 leaf path；其余 22 项已用“前后最近的已映射 leaf 必须属于同一卷”的确定性规则补齐，卷四/六/八/九分别为 1/2/10/9 项。21 个旧 hint 与边界证据冲突，继续只作证据、不参与赋值。逐段测试证明正文只发生 288 处确定性的 CRLF-to-LF 规范化。
...
**并行发布阻断项：** 22 项卷映射已由可复现的双侧同卷边界规则闭环，不再允许 runtime 静默漏段；楞严 authoritative source/rights/text review 仍是人工治理工作，继续阻止 canonical promotion 和正式新渠道发布。Android 自身 fixture adapter 已在 Phase 2/Gate C 完成，不能循环地作为创建 Android 工程之前的 F2 条件。

**Gate F2（本地工程 Gate 已通过）：** 独立 validator 可在 macOS/Linux 校验楞严；schema v1、8 类 fixtures、iOS adapter 和迁移报告已 review。Node 22.17.1 公共 Gate 通过，Xcode 26.6/iOS 26.5 Simulator 共 114 项测试（1 项真实 ODR 环境测试按预期跳过）零失败。Android adapter、生成资产和自身 Gate C 已在后续 Phase 中落地；托管 CI 证据、权威正文与发行权利仍分别受 F1 和 release Gate 管理。
...
- Android 音频 Phase 4 已开始但没有重做 iOS 播放器：第一条基础切片解析并交叉校验 Android delivery/artifact contracts，以 release state、catalog/rights、HTTPS、唯一 primary、OS 和 Range 形成启用门禁，并把 ADR 0002 的计费网络、下一卷预取、192/512 MiB、28 天和 LRU 规则变为 JVM 可测策略。楞严仍因主 host、格式实机基准和 Android 再分发权保持 `planned`，尚未声明 Media3 service/权限或显示用户入口。
- Android 音频第二条切片固定 Media3 1.10.1，在共享 media 模块完成 service-owned Player/MediaSession 与契约 ID 到可信 MediaItem 的映射；API 35 独立 harness 验证 controller 不能注入 URI，且捕获并修复了 Android ICU 才会触发的 delivery regex 崩溃。楞严正式 App 在 `planned` 期间不依赖该模块，release gate 禁止网络、媒体前台权限和 service 泄漏；该切片结束时 DownloadManager/Range 恢复、bytes/SHA、进度持久化、host/codec/权利和生产 UI 尚未完成。
- Android 音频第三条切片完成稳定的 product/artifact/rendition DownloadRequest/cache key、应用级 DownloadManager runtime 与可注入 DownloadService；API 35 回环 server 证明截断后以非零 Range 恢复，断网后的 cache-only 校验拒绝缺字节、多余尾部和错误 SHA。正式楞严 release 继续不含网络、data-sync/media-playback 前台权限、两个 Media3 service action 或 Media3 bytecode；该切片结束时坏缓存自动清除、预算/预取执行、cache-backed Player、播放位置、进程死亡、生产 host/codec/权利/UI 尚未完成。
- Android 音频第四条切片完成 application-looper 完整性协调器和后台 SHA 校验：首个坏缓存先删除并等待 `onDownloadRemoved`，再经产品注入入口修复一次；替换内容仍错误时再次清除并最终拒绝，不会无限重下。API 35 已证明等长错误内容一次修复后 verified，以及持续错误时恰好两次网络请求、DownloadIndex 清空且缓存为 0；事件线程、迟到 callback generation 和已验证下载被外部删除后的信任失效也被约束。该切片结束时持久 cache policy/计费网络执行、cache-backed Player、播放位置、进程死亡、生产 host/codec/权利/UI 尚未完成；当前进度见下一条，楞严正式 App 的隔离 gate 不变。
- Android 音频第五条切片完成 Media3 schema-v1 缓存账本和 worker-only 串行策略执行器：按完整预期字节预留，先过期再 LRU，拒绝同 request ID 合同漂移及 protected artifact 的 rendition 替换；真实 evictor 等待 `onDownloadRemoved` 后才确认索引和 span 均消失。API 35 已证明带真实 span 的 verified 记录跨 `SimpleCache` 重建，以及 28 天边界一次清空 DownloadIndex/cache/metadata；512 MiB 只约束同一独立 App runtime，不跨产品 App 沙箱。Media3 会清掉没有 span 的纯 metadata 排队项，后续进程恢复需从 catalog/DownloadIndex 重建；计费网络执行、cache-backed Player、位置、完整进程死亡、生产 host/codec/权利/UI 仍未完成，楞严正式 App 隔离不变。
- Android 音频第六条切片以 schema-v1 request marker 和应用专属 stop reason 完成逐任务计费网络执行；全局 Media3 requirement 仍只要求联网，因此用户主动播放不受“仅非计费网络”限制。自动预取在计费/离线时保留 stopped 任务，策略恢复只清除自身 reason；用户点同一卷会在相同 request/cache key 上单调提升，迟到 prefetch callback 和完整性修复都不能丢失 user/prefetch 意图。API 35 证明 metered prefetch 在提升前没有 HTTP body 或缓存字节，runtime 重建后 marker/reason 保持，用户提升后以单个 body request 完成。正式楞严仍不依赖 media；产品级网络/偏好观察、startup reservation 重建、cache-backed Player、位置、完整 OS 进程死亡、host/codec/权利/UI 仍未完成。
- Android 音频第七条切片完成 worker-only 启动 reservation 恢复：DownloadManager 初始化、idle 且仍暂停时，逐项以当前 catalog 严格校验 DownloadIndex 的 URI/MIME/cache key/完整 progressive 合同和 user/prefetch marker，再经既有预算执行器恢复 SimpleCache 重建时丢失的零字节 metadata；已存在记录不刷新 last-access，冷启动不会推迟 28 天 expiry。任何未知、漂移、重复、删除中或 admission 问题都令 `readyToResume=false`，catalog 校验失败不产生部分写入或 LRU 清理，reconciler 自身不启动下载。API 35 已证明 stopped metered prefetch 的 reservation 在 cache runtime 重建后丢失、随后从 catalog/index 精确恢复，resume 后仍为 0 HTTP/0 bytes；这不是 OS 杀进程证据。正式楞严仍不依赖 media；产品 composition 接线、完成任务的启动完整性登记、实时网络/偏好、cache-backed Player、位置、完整 OS 进程死亡、host/codec/权利/UI 仍未完成。
- Android 音频第八条切片完成可复用启动 composition：single-use application-looper coordinator 在 initialized/idle/paused 时建立，将 reconcile 调度到 worker，返回后再次确认 DownloadManager 仍 initialized/paused，再原子登记全部持久任务并对历史 completed 项主动启动 cache-only bytes/SHA 校验，最后才 resume；不要求 reconcile 自身 eviction 后再出现第二次 idle edge，也不以完整 hash 阻塞启动。调度、reconcile、登记或 resume 任一步失败都 typed fail-closed，worker 未完成前 release 会丢弃迟到结果且绝不 resume。JVM 已覆盖 catalog/transfer 批次重复、缺失和 transitional 的无副作用预检；API 35 已证明 stopped prefetch 经完整编排仍为 0 HTTP/0 bytes、完整缓存跨 runtime 重建后在 resume 前登记并启动校验，随后校验完成且不增加请求/不刷新 last-access，以及取消后 0 callback/仍 paused。这不是 OS 杀进程证据。正式楞严仍不依赖 media；实时网络/偏好、cache-backed Player、位置、完整 OS 进程死亡、host/codec/权利/UI 仍未完成。

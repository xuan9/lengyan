...
- Android 音频第二条切片固定 Media3 1.10.1，在共享 media 模块完成 service-owned Player/MediaSession 与契约 ID 到可信 MediaItem 的映射；API 35 独立 harness 验证 controller 不能注入 URI，且捕获并修复了 Android ICU 才会触发的 delivery regex 崩溃。楞严正式 App 在 `planned` 期间不依赖该模块，release gate 禁止网络、媒体前台权限和 service 泄漏；DownloadManager/Range 恢复、bytes/SHA、进度持久化、host/codec/权利和生产 UI 仍未完成。
- Android 音频第三条切片完成稳定的 product/artifact/rendition DownloadRequest/cache key、应用级 DownloadManager runtime 与可注入 DownloadService；API 35 回环 server 证明截断后以非零 Range 恢复，断网后的 cache-only 校验拒绝缺字节、多余尾部和错误 SHA。正式楞严 release 继续不含网络、data-sync/media-playback 前台权限、两个 Media3 service action 或 Media3 bytecode；坏缓存自动清除、预算/预取执行、cache-backed Player、播放位置、进程死亡、生产 host/codec/权利/UI 仍未完成。

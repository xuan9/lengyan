...
**日期：** 2026-07-27\
**版本：** v7（同步生成式开源运行时清单）\
**状态：** Android 实施中；楞严基准版已进入 Phase 5，音频、Android 反馈 UI 与完整设备发行 Gate 待完成\
...
Widget 必须验证不同 launcher、系统字号、简繁和深色背景。Widget 数据使用独立、版本化 snapshot；同一产品、内容版本和本地日期内锁定稳定段落，阅读进度或收藏变化不能让当天经句跳变。语言/主题变化刷新当前段落的呈现，日期和内容版本变化重新选择，失败时保留上一份有效数据。
...
Codex 已为当前发行运行时生成可审计的第三方软件清单，并能发现未知依赖、源码漂移和陈旧平台资源；它仍不能替代经文校勘、资产权利审核或法律判断。
...
**Phase 4 实施检查点（2026-07-27）：** 第一条非 UI 音频基础切片已完成。Android 现直接解析产品的 typed delivery contract，并与 product/platform state、artifact manifest、全局 selected rendition 和 artifact key 做交叉校验；运行时 resolver 只在 delivery active、catalog/rights release-ready、唯一兼容 HTTPS primary、配置 URL 合法且 Range 已验证时产出可播放资源。ADR 0002 的用户主动播放可用计费网络、自动下一卷默认仅非计费网络、192 MiB 单产品/同一 App runtime 512 MiB soft budget、当前/下一卷保护、28 天 expiry 和 unprotected LRU 已转成 media 模块纯函数及 JVM 测试。各产品是独立 Android 沙箱，512 MiB 不是也不能冒充跨 App 设备配额。楞严配置仍保持 `planned`，且 blocker 已从完成的缓存决策改为真实的 Android 录音再分发权确认；因此本切片没有声明网络/前台服务权限、没有显示假音频入口，也没有把现有 iOS Cloudflare emergency endpoint 标成 Android 主站。下一切片才引入固定版本 Media3 `MediaSessionService`/`DownloadService`、本地 Range 测试服务器和 bytes/SHA 验证；生产 host、格式实机基准、权利批准和 Gate E 真机证据继续阻止发布。
...
**Phase 4 下载实施检查点（2026-07-27）：** product-neutral media 模块现以 `productID:artifactID:renditionID` 同时生成 Media3 `DownloadRequest` ID 与 custom cache key，应用级 runtime 统一持有 `StandaloneDatabaseProvider`、`SimpleCache` 和单并发 `DownloadManager`，具体产品通过前台 `DownloadService` 薄子类注入 runtime、通知和 scheduler。API 35 harness 将保持 HTTPS 的合同资源仅在测试 `ResolvingDataSource` 中映射到回环 HTTP server：首个 512 KiB 响应故意截断，只有观察到非零 `Range` 重试并完整下载才通过。服务器关闭后，校验器以无 upstream 的 `CacheDataSource` 核对全部 span、Media3 content length 和 SHA-256；少一字节、多余尾部、等长错误 hash 及主线程 hash 均被拒绝。楞严 release gate 继续禁止 `INTERNET`、两类音频前台权限、MediaSessionService、DownloadService action 和全部 Media3 bytecode。该检查点结束时尚未完成坏缓存自动删除/重下、cache policy 持久元数据与执行器、计费网络预取控制、cache-backed Player、播放位置、进程死亡恢复、真实 host/codec/权利及生产 UI，因此仍不能启用产品音频。

**Phase 4 完整性修复检查点（2026-07-27）：** product-neutral 协调器现只在 `DownloadManager` application looper 接收已完成且已登记的下载，在后台执行 cache-only bytes/SHA-256 校验，并以 generation 防止 untrack/release 后的迟到结果污染状态。第一次错误会先发出 repair 状态，再删除 DownloadIndex 与缓存，等待 `onDownloadRemoved` 后通过产品注入的 enqueuer 重新下载；第二次错误再次删除并进入最终 rejected，默认最多修复一次，不会无限循环。API 35 顺序 payload server 已证明“等长错误→正确”恰好两次请求后 verified，以及“持续等长错误”恰好两次请求后 rejected、DownloadIndex 不留记录、缓存为 0；用户或未来 cache policy 删除已验证下载时，内存 verified 与旧修复预算也会立即失效。全部完整性事件均在主 application looper。前台 `DownloadService` 的 Range 恢复与协调器状态机分别测试，以保持生产级 application-scoped singleton 生命周期。该检查点当时尚未完成 cache policy 持久元数据/执行器、计费网络预取控制、cache-backed Player、播放位置、进程死亡恢复、真实 host/codec/权利和生产 UI；当前进度见下一检查点，楞严正式 release 仍不包含 Media3 或网络/音频服务权限。
...
**Phase 4 启动恢复检查点（2026-07-27）：** worker-only reconciler 在 DownloadManager 已初始化、idle 且仍暂停时读取全部 DownloadIndex，要求每个持久任务严格匹配当前产品 catalog 的 request ID、URI、MIME、custom cache key 和完整 progressive 合同，并只接受 schema-v1 user/prefetch marker 或明确的 legacy user。删除/重启中、重复、未知、合同漂移或 admission 失败都会阻止 `readyToResume`；catalog/index 校验存在问题时不写部分 metadata、不触发 LRU，所有有效持久任务在恢复批次中临时受保护。零字节 reservation 仍通过既有 192/512 MiB 预算执行器重建，已存在记录不刷新 last-access，因此冷启动不会推迟 28 天 expiry；reconciler 不调用 resume。API 35 已证明 metered stopped prefetch 的 `RESERVED` metadata 在 cache runtime 重建后消失但 DownloadIndex marker/reason 保持，恢复后即使 resume 仍为 0 HTTP request、0 cache bytes；这不是 OS 杀进程。正式楞严仍不依赖 media；该检查点结束时产品 composition 还需按“runtime/策略协调器→初始化并 idle→worker reconcile→完整性登记→resume”接线，并补实时网络/偏好、cache-backed Player、播放位置、完整进程死亡、真实 host/codec/权利和生产 UI；下一检查点完成前者。

**Phase 4 启动编排检查点（2026-07-27）：** single-use application-looper coordinator 已把“initialized/idle/paused 时建立→worker reconcile→回到 application looper 重验 initialized/paused→原子完整性登记并启动 completed 校验→resume”收敛为共享 composition 边界；reconcile 可能自行完成 cache eviction，因此不错误要求第二次 idle edge，完整 hash 也不会阻塞启动。批量登记会先验证完整 catalog/transfer 映射及既有 tracked contract，任何重复、缺失、transitional 或冲突在状态变更前整体拒绝；历史 `COMPLETED` 下载会主动开始 cache-only bytes/SHA 校验，因为冷启动不会重放旧 completion callback。调度、reconcile、DownloadManager 前置条件、登记和 resume 失败均返回 typed 结果并保持 fail-closed；reconcile 尚未回传时 release 会忽略迟到结果、0 callback 且绝不 resume。JVM 覆盖批次预检；API 35 已证明 stopped metered prefetch 经完整启动仍 0 HTTP/0 bytes，完整缓存与 DownloadIndex 保留的 runtime 重建会在 resume 前登记并启动 completed 校验，随后校验完成且无第二次请求、无 last-access 刷新，取消路径仍 paused。这不是 OS 杀进程。可复用启动 composition 与 completed 启动登记已完成；正式楞严仍因 delivery `planned` 不实例化 media，实时网络/偏好观察、cache-backed Player、播放位置、完整进程死亡、真实 host/codec/权利和生产 UI 继续未完成。
